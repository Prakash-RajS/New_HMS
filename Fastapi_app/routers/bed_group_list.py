...
async def admit_patient(
    ...
    # Prevent double admission - use sync_to_async for the query
    ...
    # Find bed and group with prefetching
    try:
    ...
    # Store old occupancy for notification
    ...
    # Reload bed with related objects for notification
    ...
    if old_occupied != bed_group.occupied:
        await safe_send_bed_notification("room_occupancy_changed", bed_group, old_occupied, bed_group.occupied)
...
async def update_bed_group(group_id: int, group_update: BedGroupCreate):
    ...
    if group_update.capacity > old_capacity:
        ...
        for bed in new_beds:
            # Reload bed with relations for notification
            bed_with_relations = await sync_to_async(
                lambda bed_id: Bed.objects.select_related('bed_group').get(id=bed_id),
                bed.id
            )()
