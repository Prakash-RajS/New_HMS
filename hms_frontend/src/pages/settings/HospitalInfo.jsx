...
const HospitalInfo = forwardRef(({ data, onUpdate, onFormChange, isDirty }, ref) => {
  ...
  const isValidHospitalName = (name) => {
    if (!name) return false;
    if (name.length > 50) return false;
    if (name.trim().length < 3) return false;
    // Allow letters, numbers, spaces, hyphens, apostrophes, periods, commas, and ampersands
    if (!/^[A-Za-z0-9\s\-'.,&]+$/.test(name.trim())) return false;
    return true;
  };
  ...
  const isValidAddress = (address) => {
    ...
    // Allow letters, numbers, spaces, hyphens, apostrophes, periods, commas, slashes, and hashes
    if (!/^[A-Za-z0-9\s\-'.,/#]+$/.test(address.trim())) return false;
    return true;
  };

  // Registration Number validation
  const isValidRegistrationNumber = (regNo) => {
    if (!regNo) return true; // Optional field
    
    // Check length (max 30 characters)
    if (regNo.length > 30) return false;
    
    // Allow letters, numbers, spaces, hyphens, slashes, and periods
    if (!/^[A-Za-z0-9\s\-/.]*$/.test(regNo)) return false;
    ...
    switch (field) {
      ...
      case "hospital_name":
        if (!value || !value.trim()) {
          ...
          error = "Hospital name cannot exceed 50 characters";
        } else if (value.trim().length < 3) {
          error = "Hospital name must be at least 3 characters";
        } else if (!/^[A-Za-z0-9\s\-'.,&]+$/.test(value.trim())) {
          error = "Hospital name can only contain letters, numbers, spaces, hyphens, apostrophes, periods, commas, and ampersands";
        }
        break;
      ...
      case "website":
        if (value && value.length > 50) {
          error = "Website URL cannot exceed 50 characters";
        } else if (value && !isValidWebsite(value)) {
          error = "Please enter a valid website URL (e.g., https://www.hospital.com)";
        }
        break;
        
      case "registration_number":
        if (value && value.length > 30) {
          error = "Registration number cannot exceed 30 characters";
        } else if (value && !/^[A-Za-z0-9\s\-/.]*$/.test(value)) {
          error = "Registration number can only contain letters, numbers, spaces, hyphens, slashes, and periods";
        }
        break;
      ...
      case "address":
        if (!value || !value.trim()) {
          ...
          error = "Address can only contain letters, numbers, spaces, hyphens, apostrophes, periods, commas, slashes, and hashes";
        }
        break;
  ...
  const checkFormValidity = () => {
    // Check required fields
    if (!isValidHospitalName(data.hospital_name)) return false;
    if (!data.phone || !isValidPhoneNumber(data.phone)) return false;
    if (!data.email?.trim() || !isValidEmail(data.email)) return false;
    if (!isValidAddress(data.address)) return false;
    
    // Check optional fields if they have values
    if (data.emergency_contact && !isValidPhoneNumber(data.emergency_contact)) return false;
    if (data.gstin && !isValidGSTIN(data.gstin)) return false;
    if (data.website && !isValidWebsite(data.website)) return false;
    if (data.registration_number && !isValidRegistrationNumber(data.registration_number)) return false;
    if (data.established_year) {
      const year = parseInt(data.established_year);
      const currentYear = new Date().getFullYear();
      if (year < 1900 || year > currentYear) return false;
  ...
  const handleChange = (field, value) => {
    ...
    // Special handling for email - convert to lowercase and limit to 50 chars
    if (field === "email") {
      value = value.toLowerCase().slice(0, 50);
    }

    // Special handling for hospital_name - limit to 50 chars
    if (field === "hospital_name") {
      value = value.slice(0, 50);
    }

    // Special handling for website - limit to 50 chars
    if (field === "website") {
      value = value.slice(0, 50);
    }

    // Special handling for registration_number - limit to 30 chars
    if (field === "registration_number") {
      value = value.slice(0, 30);
    }

    // Special handling for address - limit to 150 chars
    if (field === "address") {
  ...
  useEffect(() => {
    ...
    if (!data.hospital_name?.trim()) {
      newErrors.hospital_name = "Hospital name is required";
    } else if (data.hospital_name.trim().length > 50) {
      newErrors.hospital_name = "Hospital name cannot exceed 50 characters";
    } else if (data.hospital_name.trim().length < 3) {
      newErrors.hospital_name = "Hospital name must be at least 3 characters";
    } else if (!/^[A-Za-z0-9\s\-'.,&]+$/.test(data.hospital_name.trim())) {
      newErrors.hospital_name = "Hospital name can only contain letters, numbers, spaces, hyphens, apostrophes, periods, commas, and ampersands";
    }
    ...
    // Website validation
    if (data.website && data.website.length > 50) {
      newErrors.website = "Website URL cannot exceed 50 characters";
    } else if (data.website && !isValidWebsite(data.website)) {
      newErrors.website = "Please enter a valid website URL (e.g., https://www.hospital.com)";
    }
    
    // Registration number validation
    if (data.registration_number && data.registration_number.length > 30) {
      newErrors.registration_number = "Registration number cannot exceed 30 characters";
    } else if (data.registration_number && !/^[A-Za-z0-9\s\-/.]*$/.test(data.registration_number)) {
      newErrors.registration_number = "Registration number can only contain letters, numbers, spaces, hyphens, slashes, and periods";
    }
    ...
    if (!data.address?.trim()) {
      ...
      newErrors.address = "Address can only contain letters, numbers, spaces, hyphens, apostrophes, periods, commas, slashes, and hashes";
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                Hospital Name * <span className="text-xs text-gray-500">(max 50 chars)</span>
              </label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                  <Building className="text-gray-400" size={18} />
                ...
                <input
                  ...
                  required
                  maxLength={50}
                  placeholder="Multispeciality Hospital (max 50 chars)"
                  disabled={!isAdmin}
                  readOnly={!isAdmin}
                />
              </div>
              <div className="flex justify-between">
                <p className={`text-red-500 text-xs mt-1 ${errors.hospital_name ? '' : 'invisible'}`}>{errors.hospital_name || '\u00A0'}</p>
                <p className="text-xs text-gray-500 mt-1">{(data.hospital_name?.length || 0)}/50</p>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                Email Address * <span className="text-xs text-gray-500">(max 50 chars)</span>
              </label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                  <Mail className="text-gray-400" size={18} />
                ...
                <input
                  ...
                  required
                  maxLength={50}
                  placeholder="hospital@example.com (max 50 chars)"
                  disabled={!isAdmin}
                  readOnly={!isAdmin}
                />
              </div>
              <div className="flex justify-between">
                <p className={`text-red-500 text-xs mt-1 ${errors.email ? '' : 'invisible'}`}>{errors.email || '\u00A0'}</p>
                <p className="text-xs text-gray-500 mt-1">{(data.email?.length || 0)}/50</p>
              </div>
            </div>

            <div className="space-y-2">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                Website <span className="text-xs text-gray-500">(max 50 chars)</span>
              </label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                  <Globe className="text-gray-400" size={18} />
                ...
                <input
                  ...
                  maxLength={50}
                  placeholder="https://www.hospital.com (max 50 chars)"
                  disabled={!isAdmin}
                  readOnly={!isAdmin}
                />
              </div>
              <div className="flex justify-between">
                <p className={`text-red-500 text-xs mt-1 ${errors.website ? '' : 'invisible'}`}>{errors.website || '\u00A0'}</p>
                <p className="text-xs text-gray-500 mt-1">{(data.website?.length || 0)}/50</p>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                Registration Number <span className="text-xs text-gray-500">(max 30 chars)</span>
              </label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                  <FileDigit className="text-gray-400" size={18} />
                ...
                <input
                  ...
                  maxLength={30}
                  placeholder="AP/S3/S/S/ (max 30 chars)"
                  disabled={!isAdmin}
                  readOnly={!isAdmin}
                />
              </div>
              <div className="flex justify-between">
                <p className={`text-red-500 text-xs mt-1 ${errors.registration_number ? '' : 'invisible'}`}>{errors.registration_number || '\u00A0'}</p>
                <p className="text-xs text-gray-500 mt-1">{(data.registration_number?.length || 0)}/30</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              Full Address * <span className="text-xs text-gray-500">(max 150 chars)</span>
            </label>
            <div className="relative">
              <div className="absolute left-3 top-3 pointer-events-none">
                <MapPin className="text-gray-400" size={18} />
              ...
              <textarea
                ...
                required
                placeholder="Street, City, State, PIN Code (max 150 chars)"
                disabled={!isAdmin}
                readOnly={!isAdmin}
              />
            </div>
            <div className="flex justify-between">
              <p className={`text-red-500 text-xs mt-1 ${errors.address ? '' : 'invisible'}`}>{errors.address || '\u00A0'}</p>
              <p className="text-xs text-gray-500 mt-1">{(data.address?.length || 0)}/150</p>
            </div>
          </div>
        </div>
      </div>
