...
const BillingPreview = () => {
  ...
  const [patientInfo, setPatientInfo] = useState({
    ...
    billingStaffID: "",
    paymentMode: "",
    paymentType: "Full Payment", // Default to Full Payment
    paymentStatus: "",
    bedGroup: "",
    bedNumber: "",
  });
  ...
  const [taxConfig, setTaxConfig] = useState({
    editable: true, // Make tax editable
  ...
  // Updated Payment Types with Partial Payment
  const paymentTypes = [
    "Full Payment",
    "Partial Payment",
    "Insurance",
  ...
  useEffect(() => {
    fetchPatients();
    fetchStaffInfo();
    // Try to load tax configuration from settings
  ...
  // Effect to handle Partial Payment UI
  useEffect(() => {
    if (patientInfo.paymentType === "Partial Payment") {
      // Initialize partial payment data
  ...
  const fetchTaxConfiguration = async () => {
    try {
      // Try to fetch tax configuration from settings
      const response = await api.get("/settings/tax-configuration");
      if (response.data && response.data.taxRate) {
        setTaxConfig(prev => ({
          ...prev,
          taxRate: response.data.taxRate,
          editable: response.data.editable || true,
        }));
      }
    } catch (error) {
      console.log("Using default tax configuration");
      // Use default 18% if API fails
    }
  };
  ...
  const fetchTreatmentCharges = async (patientId) => {
    try {
      ...
      if (res.data && res.data.charges && res.data.charges.length > 0) {
        ...
        // FIX FOR TC_045: Proper S.No sequencing
        const treatmentChargesItems = res.data.charges.map((charge, idx) => ({
          chargeId: charge.id,
          sNo: (idx + 1).toString().padStart(2, "0"), // Start from 01
          description: charge.description,
          quantity: charge.quantity.toString(),
          unitPrice: charge.unit_price.toString(),
          amount: charge.amount.toString(),
          isFromTreatmentCharge: true,
          isEditable: false, // Treatment charges are not editable
        }));
  ...
  // ========== BILLING ITEMS MANAGEMENT (TC_045) ==========
  const handleAddBillingItem = () => {
    const newItem = {
      ...
      isEditable: true, // Manually added items are editable
    };
    setBillingItems([...billingItems, newItem]);
  };

  const handleDeleteBillingItem = (index) => {
    // FIX FOR TC_045: Auto-reorder S.No after deletion
    const updatedItems = billingItems.filter((_, i) => i !== index);
    
    // Re-sequence S.No for all items
  ...
  const handleEditSNo = (index, value) => {
    // Validate S.No input (only numbers)
    ...
    // Sort items by S.No and re-sequence if needed
  ...
  // ========== TAX MANAGEMENT (TC_046) ==========
  const handleTaxRateChange = (value) => {
    const taxValue = parseFloat(value);
    if (!isNaN(taxValue) && taxValue >= 0 && taxValue <= 100) {
      setTaxConfig(prev => ({ ...prev, taxRate: taxValue }));
    }
  };
  ...
  const calculateTax = () => {
    ...
    return (subtotal * (taxConfig.taxRate / 100)).toFixed(2);
  };
  ...
  // ========== MAIN BILL GENERATION ==========
  const handleGenerateBill = async () => {
    // Validation for Partial Payment (TC_047)
    if (patientInfo.paymentType === "Partial Payment") {
      const paidAmount = parseFloat(partialPaymentData.paidAmount) || 0;
    ...
    try {
      ...
      const invoiceData = {
        ...
        tax_percent: taxConfig.taxRate, // Use configurable tax rate (TC_046)
        transaction_id: null,
        payment_date: patientInfo.paymentStatus === "Paid" ? today : null,
        // Add partial payment data if applicable (TC_047)
  ...
  const renderBillingTableContent = () => {
    ...
    return billingItems.map((item, index) => (
      ...
      >
        ...
        <td className="p-2">
          <div className="flex items-center gap-1">
            <input
              ...
              disabled={item.isFromTreatmentCharge} // Treatment charges are read-only
            />
            {!item.isFromTreatmentCharge && (
              <button
        ...
        {/* Description (Editable for manual items) */}
        <td className="p-2">
          <input
            type="text"
            value={item.description}
            onChange={(e) => handleEditBillingItem(index, "description", e.target.value)}
            readOnly={item.isFromTreatmentCharge}
        ...
        {/* Quantity (Editable for manual items) */}
        <td className="p-2">
          <div className="flex items-center gap-1">
            {!item.isFromTreatmentCharge && (
              <button
        ...
        {/* Unit Price (Editable for manual items) */}
        <td className="p-2">
          <input
            type="text"
            value={item.unitPrice}
  ...
  // ========== RENDER PARTIAL PAYMENT SECTION (TC_047) ==========
  const renderPartialPaymentSection = () => {
    ...
    return (
      <div className="mt-6 p-4 border border-[#0EFF7B] dark:border-[#0EFF7B1A] rounded-lg bg-[#0EFF7B0A]">
        <h4 className="text-[#08994A] dark:text-[#0EFF7B] font-medium mb-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <input
              ...
              onChange={(e) => handlePartialPaymentChange(e.target.value, "paidAmount")}
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                onChange={(e) => handlePartialPaymentChange(e.target.value, "dueDate")}
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => handlePartialPaymentChange(e.target.value, "remarks")}
              className="w-full h-[33px] px-3 rounded-[8px] border border-[#0EFF7B] dark:border-[#3A3A3A] 
                bg-transparent text-[#08994A] dark:text-white outline-none"
    ...
    <div className="w-full max-w-screen-2xl mb-4 mx-auto">
      <div className=" mb-4 bg-gray-100 dark:bg-black text-black dark:text-white dark:border-[#1E1E1E] rounded-xl p-6 w-full max-w-[2500px] mx-auto flex flex-col overflow-hidden relative font-[Helvetica]">
        ...
        <div className="bg-[#F5F6F5] dark:bg-transparent border border-[#0EFF7B] dark:border-[#3C3C3C] rounded-xl p-4">
          <div className="flex justify-between items-center mb-3">
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className="flex items-center gap-1 text-[#0EFF7B] text-sm hover:opacity-80"
              >
                <Plus size={16} />
                Add Item
              </button>
          ...
          {/* Tax Configuration (TC_046) */}
        {/* Tax Configuration Section */}
<div className="mb-6 p-4 border border-[#0EFF7B] dark:border-[#0EFF7B1A] rounded-lg bg-[#0EFF7B0A]">
  <div className="flex flex-wrap items-center justify-between gap-6">
    <div className="flex-1 min-w-[200px]">
      <label className="text-sm text-gray-600 dark:text-gray-300 block mb-2 font-medium">
        Tax Rate (%) – {taxConfig.editable ? "Editable" : "Fixed"}
      </label>
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1 max-w-[180px]">
          <input
            ...
            onChange={(e) => {
              ...
              if (val === "" || /^\d*\.?\d{0,2}$/.test(val)) {
                setTaxConfig(prev => ({
                  ...prev,
                  customTaxRate: val,
                  // Temporarily allow invalid for typing, enforce on blur
                  taxRate: val === "" ? 0 : parseFloat(val) || 0,
                }));
              }
            }}
            onBlur={() => {
              const num = parseFloat(taxConfig.customTaxRate);
              
              if (isNaN(num) || taxConfig.customTaxRate === "") {
              ...
              // Enforce 0–100 range
              if (num < 0) {
                errorToast("Tax rate cannot be below 0%");
                setTaxConfig(prev => ({
                  ...prev,
                  customTaxRate: "0",
                  taxRate: 0,
                ...
                errorToast("Tax rate cannot exceed 100%");
                setTaxConfig(prev => ({
                  ...prev,
                  customTaxRate: "100",
                  taxRate: 100,
            ...
            placeholder="Tax %"
            className={`w-full h-[38px] px-3 rounded-lg border ${
              taxConfig.customTaxRate !== "" && 
              (parseFloat(taxConfig.customTaxRate) < 0 || parseFloat(taxConfig.customTaxRate) > 100)
                ? "border-red-500 focus:ring-red-500"
                : "border-[#0EFF7B] dark:border-[#3A3A3A] focus:ring-[#0EFF7B]"
            } bg-white dark:bg-transparent text-[#08994A] dark:text-white outline-none focus:ring-2`}
            disabled={!taxConfig.editable}
          />
          
          {/* Live error display under input */}
          {taxConfig.customTaxRate !== "" && (
            <>
              {parseFloat(taxConfig.customTaxRate) < 0 && (
                <p className="text-red-600 dark:text-red-400 text-xs mt-1">
                  Tax rate cannot be below 0%
                </p>
              )}
              {parseFloat(taxConfig.customTaxRate) > 100 && (
                <p className="text-red-600 dark:text-red-400 text-xs mt-1">
          ...
          {/* Partial Payment Section (TC_047) */}
          ...
          <div className="flex justify-end items-center mt-6 pr-4 gap-4 w-full overflow-x-hidden no-scrollbar">
            ...
            >
              <div className="flex flex-col justify-center flex-1 pl-5 pr-6 py-3 text-sm font-medium text-white gap-2">
                ...
                {/* Show Partial Payment Summary if applicable */}
                {patientInfo.paymentType === "Partial Payment" && (
                  <>
                    <div className="flex justify-between w-full">
