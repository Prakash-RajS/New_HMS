...
const generateConsolidateInvoice = async () => {
  ...
  const createTreatmentCharges = async (patientInternalId, billingItemsData, paymentStatus, paymentType) => {
    try {
      ...
      for (const item of billingItemsData) {
        ...
        const treatmentChargeData = {
          ...
          billed_amount: treatmentChargeStatus === "BILLED" ? parseFloat(item.amount) || 0 : 0,
          remaining_amount: treatmentChargeStatus === "PARTIALLY_BILLED" ? parseFloat(item.amount) || 0 : 0,
        };
        
        try {
          console.log("Creating treatment charge:", treatmentChargeData);
          const res = await api.post("/treatment-charges/", treatmentChargeData);
          createdCharges.push(res.data);
          console.log("Treatment charge created:", res.data);
        } catch (chargeError) {
          console.error(`Failed to create treatment charge for item: ${item.description}`, chargeError);
          if (chargeError.response) {
            console.error("Error response:", chargeError.response.data);
          }
        }
      }
...
const handlePaySelectedPartialInvoices = async () => {
  ...
  try {
    for (let i = 0; i < selectedPartialInvoices.length; i++) {
      const invoiceId = selectedPartialInvoices[i];

      try {
        await api.post(`/hospital-billing/invoice/${invoiceId}/mark-paid`, {
          payment_method: patientInfo.paymentMode || "Cash",
          remarks: "Bulk payment from pending invoices",
        });

        successCount++;
      } catch (err) {
        console.error(`Failed to pay invoice ${invoiceId}:`, err);
        failCount++;
    ...
    if (failCount === 0) {
      successToast(`✅ Successfully paid ${successCount} pending invoice(s)!`);
    } else {
      errorToast(
        `⚠️ Paid ${successCount} invoice(s), failed: ${failCount}\n` +
        `Failed invoices: ${failedInvoices.join(", ")}`
      );
    }
    ...
    errorToast("Failed to process bulk payment. Please try again.");
  } finally {
                  ...
                  <tr key={index} className="border-b border-gray-300 dark:border-gray-800 hover:bg-[#0EFF7B1A] dark:hover:bg-[#0EFF7B0D]">
                    <td className="p-2">
                      <input
                        ...
                        className="w-4 h-4 text-[#0EFF7B] bg-gray-100 border-gray-300 rounded focus:ring-[#0EFF7B]"
                      />
                    </td>
        ...
        >
          ...
          >
            ...
            <div className="space-y-4">
              <div>
                <label className="text-sm text-black dark:text-white block mb-1">
                  Amount to Pay ($) <span className="text-red-700">*</span>
                </label>
                <input
                  type="number"
