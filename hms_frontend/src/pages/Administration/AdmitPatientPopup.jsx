...
/* -------------------------------------------------
   AdmitPatientPopup – Only shows In-Patient records
------------------------------------------------- */
const AdmitPatientPopup = ({ onClose, onSuccess }) => {
  const [formData, setFormData] = useState({
    name: "",
  ...
  // Patient data - ONLY IN-PATIENT records
  const [allPatients, setAllPatients] = useState([]);
  const [patientNameOptions, setPatientNameOptions] = useState([]);
  const [patientIdOptions, setPatientIdOptions] = useState([]);
  const [patientFetchError, setPatientFetchError] = useState("");
  ...
  // Load bed groups AND patients (ONLY IN-PATIENT)
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch bed groups
        ...
        // Fetch patients - ONLY IN-PATIENT records
        setFetchingPatients(true);
        setPatientFetchError("");
        try {
          // Use the patients list endpoint with type filter
          const patientsRes = await api.get("/patients/", {
            params: {
              type: "in-patient", // Filter to only show in-patients
              limit: 200 // Get more records for better search
            }
          });
          console.log("Patients API response (in-patient only):", patientsRes);
          ...
          console.log("Processed in-patient list:", patientsList);

          if (patientsList.length === 0) {
            setPatientFetchError("No in-patient records found. Patients must be admitted first.");
          } else {
            setAllPatients(patientsList);
          ...
          console.error("Failed to fetch in-patient data:", patientsError);
          
          let errorMessage = "Could not load in-patient list";
          if (patientsError.response?.status === 400) {
            errorMessage = "Server error: Database connection issue";
          } else if (patientsError.response?.status === 401 || patientsError.response?.status === 403) {
            errorMessage = "Authentication required. Please refresh the page.";
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-70 font-[Helvetica] z-50">
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div>
            ...
            <TypeAheadDropdown
              ...
              error={errors.name || errors.full_name}
              placeholder={
                fetchingPatients ? "Loading in-patients..." : 
                patientNameOptions.length > 0 ? "Type patient name..." :
        ...
        {/* Info message about in-patient requirement */}
        {!fetchingPatients && patientNameOptions.length === 0 && !patientFetchError && (
          <div className="mt-4 p-3 rounded-md bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
            <div className="flex items-center gap-2">
              <span className="text-blue-700 dark:text-blue-300 text-sm">
                Only in-patient records are shown. Patients must be admitted first to be eligible for bed allocation.
              </span>
            </div>
          </div>
        )}
