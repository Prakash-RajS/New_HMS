...
const EditBedGroupPopup = ({ onClose, onUpdate, data }) => {
  ...
  const validateBedFromFormat = (value) => {
    if (!value.trim()) return "";
    if (isNaN(value) || parseInt(value) <= 0) return "Starting bed number must be a positive number";
    if (parseInt(value) > 999) return "Starting bed number cannot exceed 999";
    return "";
  };

  const validateBedToFormat = (value) => {
    if (!value.trim()) return "";
    if (isNaN(value) || parseInt(value) <= 0) return "Ending bed number must be a positive number";
    if (parseInt(value) > 999) return "Ending bed number cannot exceed 999";
    if (formData.bedFrom.trim() && parseInt(value) <= parseInt(formData.bedFrom)) {
  ...
  const checkForDuplicates = async () => {
    ...
    try {
      const response = await api.post("/bedgroups/check-range", {
        bedFrom: fromNum,
        bedTo: toNum,
        excludeGroupId: data?.id, // Exclude current group when checking duplicates
      });
  ...
  const handleUpdate = async () => {
    ...
    if (!checkResult.available && checkResult.duplicates?.length > 0) {
  ...
  const proceedWithUpdate = async () => {
    ...
    try {
      ...
      console.error("Update BedGroup Error:", error);
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-70 z-50 font-[Helvetica]">
        <div className="w-[420px] h-auto rounded-[20px] bg-gray-100 dark:bg-[#000000E5] text-black dark:text-white p-6 shadow-md backdrop-blur-md relative">
          {/* Gradient Border */}
          <div
            style={{
              position: "absolute",
          ...
          <div className="flex justify-between items-center pb-3 mb-4">
            ...
            <button
              ...
              flex items-center justify-center hover:bg-[#0EFF7B33] transition"
            >
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="text-right text-xs text-gray-500 mt-1">
                {formData.bedGroupName.length}/50
              </div>
                    ...
                    className={`w-full h-[33px] px-3 rounded-[8px] border 
                      ${
                        focusedField === "bedFrom"
                          ? "border-[#0EFF7B] ring-1 ring-[#0EFF7B]"
                          : (validationErrors.bedFrom || fieldErrors.bedFrom)
                          ? "border-red-500 ring-1 ring-red-500"
                          : "border-[#0EFF7B] dark:border-[#3A3A3A]"
                      }
                      bg-gray-100 dark:bg-black text-[#08994A] dark:text-[#0EFF7B] placeholder-gray-500 
                  ...
                  <input
                    ...
                    className={`w-full h-[33px] px-3 rounded-[8px] border 
                      ${
                        focusedField === "bedTo"
                          ? "border-[#0EFF7B] ring-1 ring-[#0EFF7B]"
                          : (validationErrors.bedTo || fieldErrors.bedTo)
                          ? "border-red-500 ring-1 ring-red-500"
                          : "border-[#0EFF7B] dark:border-[#3A3A3A]"
                      }
                      bg-gray-100 dark:bg-black text-[#08994A] dark:text-[#0EFF7B] placeholder-gray-500 
          ...
          <div className="w-[500px] rounded-[19px] bg-gray-100 dark:bg-[#000000] p-6 relative">
            ...
            <div className="mb-4">
              <p className="text-gray-700 dark:text-gray-300 mb-2">
                ...
                already exist in other bed groups:
              </p>
