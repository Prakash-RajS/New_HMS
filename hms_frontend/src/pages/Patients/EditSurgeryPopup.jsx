...
import { X, Calendar, Clock, ChevronDown, AlertCircle, User, Stethoscope } from "lucide-react";
...
export default function EditSurgeryPopup({ onClose, surgery, onUpdate }) {
  ...
  // Helper to check if a given datetime is in the past
  ...
  const validateRequiredFields = () => {
    ...
    // Validate price if status is success or failed
  ...
  const handleInputChange = (field, value) => {
    ...
    // Special handling for status field – need to revalidate date/time with new status
    if (field === "status") {
      setFormData(prev => {
        const newForm = { ...prev, [field]: processedValue };
        // After updating status, validate date/time with new status
        const dateTimeError = validateDateTime(newForm.scheduled_date, newForm.scheduled_time, processedValue);
  ...
  // Get combined error for a field - always returns a string
  const getFieldError = (fieldName) => {
    ...
    // If it's an object, try to extract a string message
            ...
            <div className="col-span-1">
              ...
              <div className="relative mt-1">
                <input
                  ...
                  onBlur={() => setFocusedField(null)}
                  // No min attribute – allow any date
            ...
            <div className="col-span-1">
              ...
              <div className="relative mt-1">
                <input
                  ...
                  onBlur={() => setFocusedField(null)}
                  // No min attribute – allow any time
                  className={`w-full h-[33px] px-3 pr-10 rounded-[8px] border
                            bg-gray-100 dark:bg-transparent outline-none
                            text-black dark:text-[#0EFF7B] cursor-pointer
                            appearance-none
