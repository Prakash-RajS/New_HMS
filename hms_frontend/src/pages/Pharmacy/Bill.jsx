...
const Bill = () => {
  ...
  try {
    ...
    const medicineItems = (res.data.items || []).filter(item => 
      item.medicine_name || item.name_of_drug // Only include medicine items
    ).map((item, i) => ({
      sNo: (i + 1).toString(),
    ...
    if (medicineItems.length === 0) {
      if (fromDate || toDate) {
        errorToast("No medicine allocations found for the selected date range");
      } else {
...
const generateBill = async () => {
  ...
  try {
    ...
    // If stock is available, create invoice
    const response = await axios.post(
      `${API_BASE}/create-pharmacy-invoice/`, 
      invoiceData, 
      {
        responseType: "blob",
      }
    ...
    <div className="relative min-w-[140px] w-[160px] lg:w-[180px]">
      <Listbox
        ...
        onChange={(v) =>
          ...
          handlePatientSelect(patients.find((p) => p.patient_unique_id === v))
        }
      >
        <Listbox.Button className="w-full h-[33.5px] rounded-[8.38px] border-[1.05px] border-[#0EFF7B] dark:border-[#3C3C3C] bg-[#F5F6F5] dark:bg-black text-[#08994A] dark:text-white px-3 pr-8 text-sm font-helvetica text-left relative">
  ...
  >
    Clear Dates
        ...
        <div className="bg-[#F5F6F5] dark:bg-transparent border border-[#0EFF7B] dark:border-[#3C3C3C] rounded-xl p-4">
          ...
          {loading ? <p>Loading medicines...</p> : billingItems.length === 0 ? (
            (dateFrom || dateTo) ? <p>No medicines allocated for the selected date range.</p> : <p>No medicines allocated to this patient.</p>
          ) : (
          ...
          <div className="mt-6 flex justify-end gap-3">
            <button onClick={handleCancel} className="px-6 py-2 border border-[#0EFF7B] dark:border-[#0EFF7B1A] rounded-md text-gray-600 dark:text-gray-300 hover:text-[#08994A] dark:hover:text-white">Cancel</button>
            <button
        ...
        </div>{/* Rest of your component remains exactly the same... */}
